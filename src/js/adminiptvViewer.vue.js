import { UserSupport } from '../../src/js/userSupport.module.js?v=2.1.9'   
...
const AdminiptvViewer = {
    ...
    methods: {
        ...
        sendDemoCredentials(client) {
            this.UserSupport.sendDemoCredentials(client,(response)=>{
                if(response.s == 1)
                {
                    alertInfo({
                        icon:'<i class="bi bi-ui-checks"></i>',
                        message: `<div class="h3 text-white">Hemos enviado las credenciales al usuario</div>`,
                        size: 'modal-md',
                        _class:'bg-gradient-success text-white'
                    })
                }
        ...
        passToServiceCredentials(client) {
            this.UserSupport.passToServiceCredentials(client,(response)=>{
                if(response.s == 1)
                {
                    alertInfo({
                        icon:'<i class="bi bi-ui-checks"></i>',
                        message: `<div class="h3 text-white">Hemos enviado el mensaje de servicio al usuario</div>`,
                        size: 'modal-md',
                        _class:'bg-gradient-success text-white'
                    })
                }
        ...
        getUserPanel() {
            this.busy = true
            this.UserSupport.getUserPanel({}, (response) => {
                this.busy = false
                if (response.s == 1) {
                    this.panel = response.response

                    this.panel.permissions.allowed_pages = JSON.parse(this.panel.permissions.allowed_pages)
                }
            })
        },
        getAllIptvClientsMain() {
    ...
    {       
        this.getUserPanel()
        this.getAllIptvClientsMain()
    },
    template : `
        <div v-if="panel" class="card card-body">
            <div class="text-xs">Créditos</div>
            <div class="h3">
                {{panel.data.credits}}
            </div>

            {{panel.permissions.allowed_pages}}
        </div>
        ...
        <div v-if="clientsAux" class="card mb-4 overflow-hidden">
                        ...
                        <tbody>
                            <tr v-for="client in clients">
                                ...
                                <td class="text-center">
                                    <div v-if="client.user_name" class="mb-3">Usuario <strong>{{client.user_name}}</strong> <button @click="copyToClipboard(client.user_name,$event)" class="btn ms-2 btn-sm mb-0 btn-primary px-3">Copiar</button></div>
                                    <div v-if="client.client_password">Contraseña <strong>{{client.client_password}}</strong> <button @click="copyToClipboard(client.client_password,$event)" class="btn ms-2 btn-sm mb-0 btn-primary px-3">Copiar</button></div>
                                </td>
                                <td class="text-center">
                                    <div v-if="client.demo">
                                        <div class="d-grid mb-1"><span class="badge bg-primary">DEMO</span></div>
                                        <div class="d-grid mb-1"><span class="badge bg-danger" v-if="client.demo.adult">Canales para adultos</span></div>
                                    ...
                                    <div v-else-if="client.service">
                                        <div class="d-grid mb-1"><span class="badge bg-primary">SERVICIO</span></div>
                                        <div class="d-grid mb-1"><span class="badge bg-danger" v-if="client.service.adult">Canales para adultos</span></div>
                                        ...
                                        <ul class="dropdown-menu">
                                            <div v-if="client.service">
                                                ...
                                                <li v-else-if="client.service.status == STATUS_SERVICE.IN_USE.code">
                                                    ...
                                                    <button @click="passToServiceCredentials(client)" class="dropdown-item">Enviar credenciales de Demo a Full</button> 
