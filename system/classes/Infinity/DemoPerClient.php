...
use HCStudio\Orm;
use Infinity\ApiWhatsAppMessages;
use Infinity\ApiWhatsApp;
...
class DemoPerClient extends Orm {
  ...
  public static function expireDemo(int $client_id = null) : bool
  {
    $DemoPerClient = new DemoPerClient;

    if($DemoPerClient->loadWhere('client_id = ?',$client_id))
    {
  ...
  public static function sendDemo(array $data = null) 
  {
    ...
    if($DemoPerClient->loadWhere('demo_per_client_id = ?',$data['demo_per_client_id']))
    {
      $Client = new Client;
      ...
      if($Client->loadWhere('client_id = ?',$data['client_id']))
      {
        ...
        if($client = ApiInfinity::generateDemo($Client->user_name,$Client->client_password))
        {
